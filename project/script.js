...
function canvas1() {
  ...
  canvas.width = window.innerWidth;
  canvas.height = window.innerHeight;
  console.log('Canvas size:', canvas.width, canvas.height);  // Debugging canvas size

  window.addEventListener("resize", function () {
      canvas.width = window.innerWidth;
      canvas.height = window.innerHeight;
      console.log('Resized canvas size:', canvas.width, canvas.height);  // Debugging after resize
      render();
  });
  ...
  for (let i = 0; i < frameCount; i++) {
      ...
      img.src = files(i);
      img.onload = function () {
          console.log('Image loaded:', img.src);  // Debugging image loading
          render();
      };
      images.push(img);
  }
  ...
  function render() {
      console.log('Render function called');  // Debugging render
